...
const (
	// Jimeng limits a single uploaded file to 4.7 MB.
...
func (a *TaskAdaptor) ValidateRequestAndSetAction(c *gin.Context, info *relaycommon.RelayInfo) (taskErr *dto.TaskError) {
	if taskErr = relaycommon.ValidateMetadataPassthroughTaskRequest(c, info, constant.TaskActionGenerate); taskErr != nil {
		return taskErr
	}
...
func (a *TaskAdaptor) DoResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (taskID string, taskData []byte, taskErr *dto.TaskError) {
	...
	if reqKey := info.UpstreamModelName; reqKey != "" {
...
func (a *TaskAdaptor) FetchTask(baseUrl, key string, body map[string]any, proxy string) (*http.Response, error) {
	...
	if !ok || strings.TrimSpace(reqKey) == "" {
		return nil, fmt.Errorf("missing req_key")
	}
...
func (a *TaskAdaptor) convertToRequestPayload(req *relaycommon.TaskSubmitReq, info *relaycommon.RelayInfo) (map[string]any, error) {
	...
	reqKey = resolveJimengReqKey(reqKey, payload)
	payload["req_key"] = reqKey
	payload["frames"] = 24*taskcommon.ResolveVideoBillingDuration(*req, 5) + 1
	if info != nil {
...
func resolveJimengReqKey(reqKey string, payload map[string]any) string {
	imageLen := jimengPayloadImageLen(payload)
