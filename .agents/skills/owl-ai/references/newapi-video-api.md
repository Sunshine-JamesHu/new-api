...
- `metadata`: channel extension parameters.
...
For scripts, always send the intended billable/render duration at the top level. Channel extension duration fields may be mirrored inside metadata only when a channel requires them, but they must not be the only duration source.

## Mode Mapping Through new-api

Use `metadata` for channel-specific details:
...
- Kling:
  - Kling image-to-video uses top-level `image`.
