...
/**
 * 计费类型筛选组件
 * @param {string|'all'|'second'|0|1} filterQuotaType 当前值
...
const PricingQuotaTypes = ({
  ...
  const qtyCount = (type) =>
    models.filter((m) => {
      ...
      if (type === 'second') return m.billing_mode === 'per_second';
      if (type === 1) return m.quota_type === 1 && m.billing_mode !== 'per_second';
      return m.quota_type === type;
    }).length;
